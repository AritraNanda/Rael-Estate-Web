...
import { 
  ...
  deleteSellerStart, 
  deleteSellerSuccess 
} from "../redux/user/sellerSlice"; // Import seller actions
...
export default function SellerProfile() {
  ...
  const [displayAvatar, setDisplayAvatar] = useState(
    currentSeller?.avatar || "https://i.ibb.co/yncvwSRK/profile-circle-svgrepo-com.png"
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/user/update2/${currentSeller._id}`, {
        method: 'POST',
        headers: {
      ...
      if (data.success === false) {
        dispatch(updateSellerFailure(data.message));
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(`/api/user/delete2/${currentSeller._id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
        dispatch(deleteSellerFailure(data.message));
        toast.error(data.message);
        return;
      }
      dispatch(deleteSellerSuccess(data));
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center">
      ...
      <div className="bg-white shadow-lg rounded-[16px] overflow-hidden p-6 w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl text-center">
        <h1 className="text-3xl font-bold text-gray-700 mb-6">Seller Profile</h1>
        ...
        <div className="mt-4">
          ...
          <input
            type="text"
            id="username"
            defaultValue={currentSeller?.username || "Guest Seller"}
        ...
        <div className="mt-4">
          ...
          <input
            type="email"
            id="email"
            defaultValue={currentSeller?.email || "seller@example.com"}
            className="w-full border-2 border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            //onChange={handleChange}
            readOnly
